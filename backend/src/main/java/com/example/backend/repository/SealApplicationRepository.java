...
public interface SealApplicationRepository extends JpaRepository<SealApplication, Long> {
         ...
         * @return 待审批申请列表
         */
        @Query("SELECT sa FROM SealApplication sa, Seal s WHERE sa.sealName = s.name " +
                        "AND s.keeper = :keeper AND sa.status = 'PENDING' ORDER BY sa.applyTime ASC")
        Page<SealApplication> findKeeperPendingApplications(@Param("keeper") String keeper, Pageable pageable);
         ...
         * @return 审批时长范围统计列表
         */
        @Query("SELECT TIMESTAMPDIFF(HOUR, sa.applyTime, sa.approveTime) as hours, COUNT(sa) as count " +
                        "FROM SealApplication sa " +
                        "WHERE sa.approveTime IS NOT NULL " +
                        "GROUP BY TIMESTAMPDIFF(HOUR, sa.applyTime, sa.approveTime)")
        List<Object[]> getApprovalDurationRanges();
         ...
         * @return 审批最快的申请
         */
        @Query("SELECT sa FROM SealApplication sa " +
                        "WHERE sa.approveTime IS NOT NULL " +
                        "ORDER BY TIMESTAMPDIFF(MINUTE, sa.applyTime, sa.approveTime) ASC LIMIT 1")
        Optional<SealApplication> findFastestApprovedApplication();
        ...
        @Query("SELECT sa FROM SealApplication sa " +
                        "WHERE sa.approveTime IS NOT NULL " +
                        "ORDER BY TIMESTAMPDIFF(MINUTE, sa.applyTime, sa.approveTime) DESC LIMIT 1")
        Optional<SealApplication> findSlowestApprovedApplication();
}